...
void ADIOI_LUSTRE_lock_ahead_ioctl(ADIO_File fd, int avail_cb_nodes, ADIO_Offset next_offset, int *error_code)
{
    ...
    arg = ADIOI_Malloc(sizeof(struct llapi_lock_ahead_arg) + sizeof(struct llapi_lock_ahead_extent) * num_extents);
    ...
    for (i = 0; i < num_extents; ++i) {
        if (arg->lla_extents[i].result) {
            fprintf(stderr, "%s(%d) "
                    ...
                    (long int)arg->lla_extents[i].end/stripe_size,
                    arg->lla_extents[i].result);
        }
    }
#endif
