                  ...
                  ADIO_Offset step_size)
{
        ...
        ladvise_hdr = ADIOI_Malloc(sizeof(struct llapi_ladvise_hdr) + sizeof(struct llapi_lu_ladvise) * num_advise);
      ...
      fd->hints->fs_hints.lustre.lock_ahead_end_extent  = ladvise_hdr->lah_advise[num_extents - 1].lla_end;
...
void ADIOI_LUSTRE_lock_ahead_ioctl(ADIO_File fd, int avail_cb_nodes, ADIO_Offset next_offset, int *error_code)
{
        ...
        */
        if (agg_idx != rank_index) {
            fprintf(stderr, "%s(%d) rank[%d] file system %d "
                    ...
                    (long long)next_offset/stripe_size,
                    num_extents);
        }
    ...
    err = llapi_ladvise_lock(fd, flags, num_extents,
                  &offset, stripe_size, num_extents,
                  step_size);
